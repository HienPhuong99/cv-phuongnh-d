...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'save') {
        ...
        $data = [
            ...
            'icon'         => trim($_POST['icon'] ?? 'message-square'),
  ...
  <div class="table-responsive">
    <table class="table table-dark-custom mb-0 align-middle">
      ...
      <tbody data-sortable-table="skills" data-sortable-handle=".drag-handle">
        <?php if (empty($skills)): ?>
          <tr>
            <td colspan="7" class="text-center py-4 text-secondary">Chưa có kỹ năng nào. Nhấn "Thêm kỹ năng mới" để bắt đầu.</td>
          </tr>
        <?php else: ?>
            ...
            <tr data-id="<?= $skill['id'] ?>">
              ...
              <td class="text-end pe-4">
                <button type="button" class="btn btn-sm btn-outline-warning me-1"
                        ...
                        data-icon="<?= e($skill['icon']) ?>"
                        data-color="<?= e($skill['accent_color']) ?>"
      ...
      <form method="POST" action="skills.php">
        ...
        <div class="modal-body space-y-3">
          ...
          <div class="row g-3 mb-3">
            <div class="col-md-6">
              <label class="form-label" for="skill_icon">Biểu tượng Icon</label>
              <select name="icon" id="skill_icon" class="form-select">
                <option value="message-square">Tin nhắn / Hội thoại (message-square)</option>
                <option value="star">Ngôi sao / Giải pháp (star)</option>
                <option value="search">Kính lúp / Tìm kiếm (search)</option>
                ...
                <option value="file-text">Văn bản / Excel (file-text)</option>
                <option value="message-circle">Zalo / Chat (message-circle)</option>
              </select>
            </div>
            <div class="col-md-6">
              <label class="form-label" for="skill_color">Màu nhấn</label>
              <select name="accent_color" id="skill_color" class="form-select">
...
function editSkill(btn) {
  ...
  document.getElementById('skill_icon').value = btn.getAttribute('data-icon');
  document.getElementById('skill_color').value = btn.getAttribute('data-color');
...
document.getElementById('skillModal').addEventListener('hidden.bs.modal', function () {
  ...
  document.getElementById('skill_icon').value = 'message-square';
  document.getElementById('skill_color').value = 'gold';
